/*Forked from DrTUnit5CPPHints-1
 unit 5 assigment part1 Dr.T C++ Monsivais
Date 2-15-2020 IDE:Repl.it tool*/
...
void displayLow(string);// tolower function prototype
void displayUp(string);// toupper function prototype

int main( )
{    
       displayLow("\nTCC_TR IS COOL");// function call
         displayUp("\ni will succeed at c++"); //function call

         //a function call for each person - no input from the console for part 1
         cout << fixed << setprecision(2); 
         cout << weeklyPay (47.0, 505.75,"Christian",1) << endl;
...
{// complete the function definition
 ...
 void displayLow(string lwer) 
{ // function definition
  locale loca;
  
  for(int i = 0; i < lwer.length(); i++ )
  {
    cout << tolower(lwer[i],loca); 
  
  }
} 
 void displayUp(string upr)
 {//function definition
  locale loca2;
  
  for(int i = 0; i < upr.length(); i++ )
  {
    cout << toupper(upr[i],loca2); 
